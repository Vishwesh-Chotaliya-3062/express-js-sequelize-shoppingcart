...
exports.postVerify = async function (req, res, next) {
    ...
    try {
        ...
        const verified = await Secretcode.findOne({where: { Email: user.Email, Code: secretcode }});
        if (verified) {
            user.Status = 'active'; 
            user.wallet.Balance = 500;
            await user.wallet.save({transaction: verifyTransaction});
            await user.save({transaction: verifyTransaction});
            await Secretcode.destroy({ where: { Email: user.Email }, transaction: verifyTransaction });
